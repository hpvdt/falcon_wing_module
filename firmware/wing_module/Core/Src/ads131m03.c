...
const int ADS_TIMEOUT_MS = 1000;
static struct ADSConfig _config;

static struct ADSModeRegister _ads_mode = {
...
static volatile int_fast8_t bytes_per_word = 3;
static volatile int32_t current_reading[3] = {0,0,0};

void ads_write_reg(enum ADSRegisterAddress reg, uint8_t numRegs, uint16_t *source);
void ads_read_reg(enum ADSRegisterAddress reg, uint8_t numRegs, uint16_t *destination);
 ...
 */
void ads_write_reg(enum ADSRegisterAddress reg, uint8_t numRegs, uint16_t *source) {
	uint16_t write_command_to_issue = ADS_CMD_WREG | (reg << 7) | (numRegs - 1);
	...
	*/
	...
	uint16_t calculated_crc = ads_calculate_crc(reg, source, numRegs);
	...
	for (; reg_sent < 3; reg_sent++) {
		...
		if (reg_sent < numRegs) current_reading[reg_sent] = ads_transfer_single_word(source[reg_sent], true);
		else if (reg_sent == numRegs) current_reading[reg_sent] = ads_transfer_single_word(calculated_crc, true);
		else current_reading[reg_sent] = ads_transfer_single_word(0, true);
	}

	// Read in CRC from ADC
	uint16_t received_crc = 0;
	if (reg_sent < numRegs) received_crc = ads_transfer_single_word(source[reg_sent], false);
	else if (reg_sent == numRegs) received_crc = ads_transfer_single_word(calculated_crc, false);
	else received_crc = ads_transfer_single_word(0, false);
	reg_sent++;

	// Send remaining registers. Data from ADS131M03 is invalid at this point
	for (; reg_sent < numRegs; reg_sent++) ads_transfer_single_word(source[reg_sent], false);

	// Append CRC if needed to exchanges longer than the standard message
	if ((_ads_mode.spi_crc_enable) && (numRegs > 4)) ads_transfer_single_word(calculated_crc, false);
...
void ads_read_reg(enum ADSRegisterAddress reg, uint8_t numRegs, uint16_t *destination) {
	...
	uint16_t readCommandToIssue = ADS_CMD_RREG | (reg << 7) | (numRegs - 1);

	ads_command(readCommandToIssue);

	// Read in the register(s)
	if (numRegs == 1) *destination = ads_command(ADS_CMD_NULL); // For a single register the register is returned as a response to the subsequent command
	else {
		...
		for (uint8_t i = 1; i < numRegs; i++) destination[i] = ads_transfer_single_word(0, false);
...
/**
 * @brief Copies values read in from ADC to an array
 * 
...
//	noInterrupts(); // Atomic read of ADC values
	for (int_fast8_t i = 0; i < 3; i++) destination[i] = current_reading[i];
//	interrupts();
}
